...
void OnboardWindow::on_okButton_released()
{
    if (selectedFilePath == "" || !fileExists(selectedFilePath) ) {
        QMessageBox::warning(this, tr("Warning"), tr("Chip8 ROM not selected or file is not exist."));
    } else {
        Chip8UI* chip8Emu = new Chip8UI(this);
        const char* fileNameCStr = selectedFilePath.toStdString().c_str();
        chip8Emu->readChip8ROM(fileNameCStr);
        chip8Emu->show();
    }
}
